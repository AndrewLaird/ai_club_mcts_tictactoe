...
def get_game_action_probs(mcts_model, board, turn, temp=1.0):
    ...
    action_probs = [x ** (1.0 / temp) for x in visit_counts]
...
if __name__ == "__main__":
    ...
    for train_loop in range(num_training_loops):
        ...
        print(
            "\rTies: %.2f Player 1: %.2f Player 2: %.2f                  "
            % (win_averages[0], win_averages[1], win_averages[2])
        )
        ...
        if pit_results[2] < (num_pit_games - pit_results[0]) * 0.50:
            # old_model won
